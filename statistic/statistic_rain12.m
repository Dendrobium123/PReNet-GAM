...
PReNet = '../results\Rain100L\PReNet_GAM_behindRES_12/';
...
for nnn = 1:nmodel
    ...
    for iii=nstart+1:nstart+nimgs
        for jjj=1:nrain
            ...
            x_true=im2double(imread(fullfile(gt_path,sprintf('%03d_GT.png',iii))));%x_true
            x_true = rgb2ycbcr(x_true);x_true=x_true(:,:,1);
            

            %%
            x = (im2double(imread(fullfile(struct_model{nnn}.path,sprintf('%03d_in.png',iii)))));
            x = rgb2ycbcr(x);x = x(:,:,1);
