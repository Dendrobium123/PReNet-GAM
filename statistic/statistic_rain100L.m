...
gt_path='../datasets/test/Rain100L/norain/';


PReNet = '../results/Rain100L/PReNet_GAM_behindRES_demo/';
...
for nnn = 1:nmodel
    ...
    for iii=nstart+1:nstart+nimgs
        for jjj=1:nrain
            ...
            x_true=im2double(imread(fullfile(gt_path,sprintf('%03d.png',iii))));%x_true
            x_true = rgb2ycbcr(x_true);x_true=x_true(:,:,1);
            

            %%
            x = (im2double(imread(fullfile(struct_model{nnn}.path,sprintf('%03d.png',iii)))));
            x = rgb2ycbcr(x);x = x(:,:,1);
